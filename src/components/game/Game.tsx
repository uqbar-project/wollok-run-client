...
const Game = (_: GameProps) => {
  ...
  function cantLoadProgramView(error: Error) {
    ...
    if(error instanceof TooManyProgramsException){
      ...
      <div>
        <h1> {error.message} </h1>
        <p style={{ marginTop: '5px', marginBottom: '5px' }}>Se encontraron varios programas en el proyecto.
         Seleccione uno para continuar:
        </p>
        <FormControl className = {$.wpgmPicker}>
          <NativeSelect defaultValue = "" style={{padding: '1%'}} onChange={(e) => program = e.target.value}>
        ...
        <Button onClick={() => { setError(undefined); loadGame(error.files, program) }} variant="contained" color="primary">Cargar Juego</Button>
      </div>
    </div>
    }
