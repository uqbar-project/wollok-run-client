...
const ASTDisplay = () => {
  const { environment, debuggedNode } = useContext(DebuggerContext)
  ...
  return (
    <Tree
      defaultExpandedKeys={debuggedNode.ancestors().map(({ id }) => id)}
      defaultSelectedKeys={[debuggedNode.id]}
      treeData={[toASTData(environment)]}
